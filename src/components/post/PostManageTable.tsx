...
export default function PostManageTable() {
    ...
    const [selectedPostIdForDeletion, setSelectedPostIdForDeletion] = useState<string>('')
    ...
    const publishOrUnpublishPost = async (postId: string, publish: 'true' | 'false') => {
        const params = new URLSearchParams({
            postId: postId,
            publish: publish
        })
    ...
    const mutationPublishOrUnpublishPost = useMutation({
        mutationFn: ({ postId, publish }: { postId: string, publish: 'true' | 'false' }) => publishOrUnpublishPost(postId, publish)
    })
    ...
    useEffect(() => {
        ...
        refetch()
    }, [mutationDeletePost.isSuccess, mutationPublishOrUnpublishPost.isPending, mutationPublishOrUnpublishPost.isSuccess, orderBy, pathName, refetch, replace, searchParams])
                ...
                <div className="modal-action">
                    <form method="dialog">
                        ...
                        <div className="flex justify-center gap-4">
                            <button className="btn btn-error" onClick={() => handleDeleteModalWarn('delete')}>Delete</button>
                            <button className="btn" onClick={() => handleDeleteModalWarn('cancel')}>Close</button>
                        </div>
                    </form>
                </div>
            </div>
                                ...
                                <td>
                                    <div className="flex justify-end items-center gap-4">
                                        <button className="btn btn-xs btn-outline btn-error" onClick={() => {
                                            ...
                                            setSelectedPostIdForDeletion(post.id)
                                        }}>Delete</button>
                                        ...
                                        <div className="dropdown dropdown-left">
                                            <label tabIndex={0}><FontAwesomeIcon icon={faEllipsis} className="cursor-pointer" size="lg" /></label>
