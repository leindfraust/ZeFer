...
export default function SeriesManagePostContainer({ id, title, action }: PostSeries & { action: 'add' | 'remove' }) {
    ...
    const mutationDeletePostToSeries = useMutation({
        mutationFn: ({ postId }: { postId: string }) => deletePostToSeries(postId)
    })
    ...
    }, [mutationAddPostToSeries.isSuccess, mutationDeletePostToSeries, refetch])

    return (<>
        <div className="overflow-x-auto mt-4">
            <div className="join container">
                <input type="text" placeholder="Search…" className="input input-bordered w-full join-item" value={postSearch} onChange={(e) => setPostSearch(e.currentTarget.value)} />
            ...
            <table className="table table-lg">
                <tbody>
                    {isSuccess && data && data.filter((post: Post) => post.title.toLowerCase().includes(postSearch.toLowerCase()) || post.id === postSearch).map((post: Post) => (
                        <Fragment key={post.id}>

                            <tr>
                                <th><Link href={`/${post.authorUsername || post.userId}/${post.titleId}`}>{post.title}</Link></th>
                                ...
                                <td>
                                    <div className="flex justify-end items-center gap-4">
                                        {action === 'add' ? (
                                            <button className="btn btn-xs btn-outline btn-info" onClick={() => mutationAddPostToSeries.mutate({
                                                postId: post.id
                                            })}>Add</button>

                                        ) : (
                                            <button className="btn btn-xs btn-outline btn-error" onClick={() => mutationDeletePostToSeries.mutate({
                                                postId: post.id
                                            })}>Remove</button>
                                        )}
                                    </div>
