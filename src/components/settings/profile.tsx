...
export default function ProfileSettingsComponent({ username, name, bio, address, occupation, email, socials, id, image }: User) {
    ...
    const socialForms: FormContext[] = [
        {
            ...
            value: socialData.find(social => social.name === 'Facebook')?.url ? socialData.find(social => social.name === 'Facebook')?.url : '',
            required: {
                value: false,
                message: ''
            },
        },
        {
            ...
            value: socialData.find(social => social.name === 'Twitter')?.url ? socialData.find(social => social.name === 'Twitter')?.url : '',
            required: {
                value: false,
                message: ''
            }
        },
        {
            ...
            value: socialData.find(social => social.name === 'Personal Website')?.url ? socialData.find(social => social.name === 'Personal Webite')?.url : '',
            required: {
                value: false,
                message: ''
            },
    ...
    return (<>
        <div className="mx-auto lg:w-9/12 justify-center space-y-6">
            ...
            <button className='btn btn-info w-full' onClick={updateDetails}>Update Profile Information</button>
        </div>
    </>)
}